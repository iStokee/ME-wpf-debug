...
{
    /// <summary>A window size choice: a fixed preset, or the live "Custom (WxH)" entry.</summary>
    ...
    public class ThemeSettingsViewModel : INotifyPropertyChanged
    {
        ...
        // ── Window size ───────────────────────────────────────────────────────────
        private static readonly (string Label, double W, double H)[] SizePresets =
        {
            ("1024 × 768", 1024, 768),
            ("1200 × 900 (default)", 1200, 900),
            ("1280 × 800", 1280, 800),
            ("1600 × 900", 1600, 900),
            ("1920 × 1080", 1920, 1080),
            ("2560 × 1440", 2560, 1440),
        ...
        public ObservableCollection<WindowSizeOption> WindowSizeOptions { get; } = new();
        ...
        public WindowSizeOption SelectedWindowSize
        {
            ...
            set
            {
                if (SetProperty(ref _selectedWindowSize, value) && value != null && !_suppressSizeApply && !value.IsCustom)
                {
                    ApplyWindowSize(value.Width, value.Height);
                }
            }
        }
        ...
        public ThemeSettingsViewModel()
        {
            ...
            foreach (var (label, w, h) in SizePresets)
                WindowSizeOptions.Add(new WindowSizeOption { Label = label, Width = w, Height = h });
            RefreshCurrentSize();
        }

        /// <summary>
        /// Re-sync the dropdown with the live window size: select the matching preset,
        /// or surface (and select) a "Custom (WxH)" entry when the window was resized
        /// by hand. Called on load and from the view whenever the main window resizes.
        /// </summary>
        public void RefreshCurrentSize()
        {
            ...
            try
            {
                var preset = WindowSizeOptions.FirstOrDefault(o =>
                    !o.IsCustom && Math.Abs(o.Width - w) <= 1 && Math.Abs(o.Height - h) <= 1);

                var custom = WindowSizeOptions.FirstOrDefault(o => o.IsCustom);
                if (preset != null)
                {
                    if (custom != null) WindowSizeOptions.Remove(custom);
                    SelectedWindowSize = preset;
                }
                else
                {
                    if (custom == null)
                    {
                        custom = new WindowSizeOption { IsCustom = true };
                        WindowSizeOptions.Add(custom);
                    }
                    custom.Width = w;
                    custom.Height = h;
                    custom.Label = $"Custom ({w:0} × {h:0})";
                    // Re-add to refresh the displayed label (option has no change notification).
                    var idx = WindowSizeOptions.IndexOf(custom);
                    WindowSizeOptions.RemoveAt(idx);
                    WindowSizeOptions.Add(custom);
                    SelectedWindowSize = custom;
                }
            }
            finally
            {
        ...
        private void ApplyWindowSize(double width, double height)
        {
            ...
            try
            {
                ...
                window.Width = Math.Min(width, SystemParameters.VirtualScreenWidth);
                window.Height = Math.Min(height, SystemParameters.VirtualScreenHeight);
                MESharp.Services.ThemeManager.SaveWindowSize(window.Width, window.Height);

                var custom = WindowSizeOptions.FirstOrDefault(o => o.IsCustom);
                if (custom != null && !ReferenceEquals(SelectedWindowSize, custom))
                    WindowSizeOptions.Remove(custom);
            }
            catch { /* sizing must never break the settings page */ }
        }
